...
import {
  ...
  Menu,
  ChevronLeft,
  User,   // ADDED: User icon for uploaded by
...
// NEW HELPER: Formats Date and Time
...
export default function UserDashboard() {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        // ADDED: Data mapping for consistency and to use specific fields
        const mappedData = response.data.map(doc => ({
          id: doc.id,
          filename: doc.filename || doc.name,
          size: doc.size,
          contentType: doc.contentType,
          // Use common fallbacks if primary fields are missing
          uploadedBy: doc.uploadedBy || doc.uploaderName || doc.user || "Unknown User",
  ...
  // ... (useEffect and handleFileUpload remains similar, just applying Tailwind fixes) ...
  useEffect(() => {
        ...
        return (
          <div className="max-w-6xl">
            <div className="bg-white rounded-xl shadow-lg p-8">
              <h1 className="text-3xl font-bold text-blue-900 mb-8">Welcome to DocManager</h1>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 mb-10">
                <div 
                  onClick={() => setActiveView("upload")}
                  // FIXED Tailwind: bg-linear-to-br -> bg-gradient-to-br
                  className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl cursor-pointer hover:shadow-lg transition-all border-2 border-blue-200"
                >
                ...
                <div 
                  ...
                  // FIXED Tailwind: bg-linear-to-br -> bg-gradient-to-br
                  className="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl cursor-pointer hover:shadow-lg transition-all border-2 border-green-200"
                >
                  <div className="flex items-center justify-center mb-4">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {/* ENHANCEMENT: Added Uploader and Full DateTime *
                              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-gray-500">
                                <span className="flex items-center gap-1">
                                    {formatFileSize(doc.size)}
                                </span>
                                <span className="flex items-center gap-1" title="Uploaded By">
                                  <User className="w-3 h-3" /> {doc.uploadedBy || "Unknown"}
                                </span>
                                <span className="flex items-center gap-1" title="Uploaded Time">
                                  <Clock className="w-3 h-3" /> {formatDateTime(doc.uploadedAt)}
                                </span>
                              </div>
                            </div>
                          </div>
                          <button
                            ...
                            className="ml-3 p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg"
                            title="Download"
                          >
          ...
          <div className="max-w-4xl">
            <div className="bg-white rounded-xl shadow-lg p-8">
              ...
              <div className="border-3 border-dashed border-blue-300 rounded-xl p-12 text-center mb-8 bg-blue-50">
                ...
                {/* FIXED Tailwind: bg-linear-to-r -> bg-gradient-to-r *
                <label className="inline-flex items-center px-8 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 cursor-pointer shadow-md">
            ...
            <div className="bg-white rounded-xl shadow-lg p-8">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                ...
                <div className="flex gap-4">
                 {/* FIXED Tailwind: bg-linear-to-r -> bg-gradient-to-r *
                  <button
                    onClick={() => setActiveView("upload")}
                    className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 flex items-center gap-2 font-medium"
                  >
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {documents.map((doc) => (
                    <div key={doc.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-shadow bg-white flex flex-col h-full">
                      ...
                      {/* ENHANCEMENT: Detailed Metadata Section *
                ...
                <div className="text-center py-16">
                  ...
                  <button
                    onClick={() => setActiveView("upload")}
                    className="px-8 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 font-medium"
                  >
                    Upload Documents
                  </button>
                </div>
...
export default function UserDashboard() {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        // Data mapping remains, ensuring consistency for the 'documents' view
        const mappedData = response.data.map(doc => ({
          id: doc.id,
          filename: doc.filename || doc.name,
          size: doc.size,
  ...
  useEffect(() => {
    // Only fetch documents if the user navigates to the 'documents' view.
        ...
        return (
          <div className="max-w-6xl">
            <div className="bg-white rounded-xl shadow-lg p-8">
              ...
              {/* REMOVED: The "Recent Documents" section has been removed here. */}
